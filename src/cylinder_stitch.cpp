...
// 7
cv::Mat linearStitch_parallel_for_mul_alpha(cv::Mat img1, cv::Mat img2,
                                            cv::Point2i a) {
  float d = img1.cols - a.x;     // 过渡区域的宽度
  int ms = img1.rows - abs(a.y); // 拼接图行数
  int ns = img2.cols + a.x;      // 拼接图列数
        ...
        cv::parallel_for_(
            cv::Range(a.x, img1.cols), [&](const cv::Range &range_col) {
              for (int j = range_col.start; j < range_col.end; j++) {
                ...
                if ((i + a.y) < img1.rows && (j - a.x) < img2.cols) {
                  alpha = (d - (j - a.x)) / d;
                  stitch.at<cv::Vec3d>(i, j) =
                      alpha * img1.at<cv::Vec3d>(i + a.y, j) +
                      (1 - alpha) * img2.at<cv::Vec3d>(i, j - a.x);
                }
              }
            });
      }
...
cv::Mat stitch_image(cv::Mat stitch, cv::Mat img1, cv::Mat img2, cv::Point2i a,
  ...
  case 6:
    ...
    break;
  case 7:
    stitch = linearStitch_parallel_for_mul_alpha(img1, img2, a);
    break;
  default:
    break;
  }
